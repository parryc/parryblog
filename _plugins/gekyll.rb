...
GEKYLL_DEFAULTS = {
    ...
    "extension_matches" => ["textile", "md", "mkd", "markdown", "txt" ],
...
module Jekyll
    class Post
        ...
        def self.valid?(name)
            self.in_repo?(name) or name =~ MATCHER
        end
        ...
        class << self
            ...
            def new(*args)
                ...
                if self.in_repo?(name) and self != GekyllPost
                    return GekyllPost.new(site, source, dir, name.split('/')[0])
                end
                new_orig(*args)
            end
        end
